...
import { LightUseVisit, mapEncounters, useInfiniteVisits } from './visit.resource';
import VisitsTable from './past-visits-components/visits-table';
import VisitSummary from './past-visits-components/visit-summary.component';
import styles from './visit-detail-overview.scss';
import DisplayVisitData, { type PageResult } from './DisplayVisitsData';
...
interface optimizedData {
  pageOfResults: PageResult[];
  totalCount: number;
}

function VisitDetailOverviewComponent({ patientUuid }: VisitOverviewComponentProps) {
  const [oldApiCalled, setOldApiCalled] = useState(true);
  const { data, lightError, lightIsLoading } = LightUseVisit(patientUuid);
            ...
            <>
              <TabPanel>
                {lightIsLoading ? (
                  ...
                  <ErrorState headerTitle={t('visits', 'visits')} error={lightError} />
                ) : optimizedData?.pageOfResults?.length ? (
                  optimizedData?.pageOfResults.map((result) => (
                    <div className={styles.container} key={result.visit.uuid}>
                      <div className={styles.header}>
                        <div className={styles.visitInfo}>
                          <div>
                            <h4 className={styles.visitType}>{result.visit.visitType.display}</h4>
                            <div className={styles.displayFlex}>
                              <h6 className={styles.dateLabel}>{t('start', 'Start')}:</h6>
                              <span className={styles.date}>
                                {formatDatetime(parseDate(result?.visit?.startDatetime))}
                              </span>
                              {result.visit.stopDatetime && (
                                <div>
                                  <h6 className={styles.dateLabel}>{t('end', 'End')}:</h6>
                                  <span className={styles.date}>
                                    {formatDatetime(parseDate(result?.visit?.stopDatetime))}
                                  </span>
                                </div>
                              )}
                            </div>
                          ...
                          <div>
                            <ExtensionSlot
                              ...
                              state={{ patientUuid, visit: result.visit }}
                            />
                          </div>
                        </div>
                      </div>
                      <DisplayVisitData Diagnosis={result?.diagnoses} visitNote={result?.visitNotes} />
                    </div>
                  ))
                ) : (
